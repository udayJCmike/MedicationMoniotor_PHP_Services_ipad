		...
		if($query11 = mysql_query($sql11))
		{
			...
			if($flag == '1')
			{
				...
				//	$sql12 = "SELECT * FROM tbl_medication_details WHERE md_medication_id = ".$record11['pmt_taketime_medicineid'];
                    $sql12=  "select t1.pmt_taketime_patientid,t1.pmt_taketime_fromdate,t1.pmt_taketime_todate,t1.pmt_taketime_medtype,t2.* from tbl_patientmedtaketime_details as t1 join tbl_medication_details as t2 on t1.pmt_taketime_medicineid=t2.md_medication_id where t1.pmt_taketime_patientid = '".$patient_id."' AND t2.md_medication_id=".$record11['pmt_taketime_medicineid'];
					$query12 = mysql_query($sql12);
					$row		= mysql_fetch_object($query12);

					$json 		.= '{ "serviceresponse" : { "servicename" : "Select Medicine", "success" : "Yes", "medicinename" : "'.$row->md_medicine_name.'", "medicinenotes" : "'.$row->pmt_taketime_patientid.'","fromd" : "'.$row->pmt_taketime_fromdate.'","tod" : "'.$row->pmt_taketime_todate.'", "direction" : "'.$row->md_medicine_direction.'","type" : "'.$row->pmt_taketime_medtype.'","message" : "1" } }';
